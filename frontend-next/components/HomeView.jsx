...
/** Presentational home body — receives SSR/ISR data (no client fetch waterfall). */
export default function HomeView({ articles = [], topics = [] }) {
    ...
    <div className="max-w-[1200px] mx-auto px-5 pb-12" data-testid="home-page">
      <header className="border-b-2 border-foreground py-5">
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <Link href="/search" prefetch={false} className="brutal-btn text-[10px] px-3 py-2 gap-2">
        ...
        <>
          <section className="mt-6" aria-label="Latest stories">
            <div className="flex items-center justify-between mb-3.5">
              ...
              <Link href="/search" prefetch={false} className="overline hover:text-primary" data-testid="view-all-articles">
                View all →
              </Link>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {articles.map((a, i) => (
                <ArticleCard key={a.slug} a={a} priority={i < 3} />
              ))}
            </div>
          </section>
