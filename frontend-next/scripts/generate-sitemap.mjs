/**
 * Writes public/sitemap.xml and public/google-sitemap.xml before build.
...
`;
  ...
  const googleOut = resolve(root, 'public', 'google-sitemap.xml');
  writeFileSync(googleOut, xml, 'utf8');

  console.log(`Wrote ${out} and ${googleOut} (${entries.length} URLs) for ${SITE_URL}`);
}
