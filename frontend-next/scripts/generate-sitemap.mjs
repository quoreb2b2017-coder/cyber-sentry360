/**
 * Generates static public/sitemap.xml for Google Search Console.
 */
...
`;
  ...
  if (!xml.includes('<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">')) {
    console.error('Invalid sitemap: missing urlset namespace');
    process.exit(1);
  }
  ...
  if (locs.some((loc) => !loc.startsWith('https://'))) {
    console.error('Refusing to write sitemap: all <loc> URLs must be absolute HTTPS.');
    process.exit(1);
  }
