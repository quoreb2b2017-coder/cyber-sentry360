/**
 * Writes lib/seo/generated-sitemap.xml before build for Google Search Console.
...
`;
  ...
  const xmlOut = resolve(root, 'lib', 'seo', 'generated-sitemap.xml');
  writeFileSync(xmlOut, xml, 'utf8');

  const tsOut = resolve(root, 'lib', 'seo', 'generated-sitemap.ts');
  const escaped = xml.replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\$\{/g, '\\${');
  writeFileSync(
    tsOut,
    `/** Auto-generated by scripts/generate-sitemap.mjs — do not edit manually. */\nexport const GENERATED_SITEMAP_XML = \`${escaped}\`;\n`,
    'utf8'
  );

  console.log(`Wrote ${xmlOut} (${entries.length} URLs) for ${SITE_URL}`);
}
