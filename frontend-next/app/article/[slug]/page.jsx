...
function ArticleSkeleton() {
  return (
    <div className="max-w-[1100px] mx-auto px-5 py-5 animate-pulse">
      <div className="h-3 w-28 bg-muted mb-4" />
      <div className="h-8 w-3/4 bg-muted mb-2" />
      <div className="h-8 w-1/2 bg-muted mb-4" />
      <div className="aspect-[2/1] w-full bg-muted border-2 border-foreground mb-5" />
      <div className="space-y-2">
        {[1, 2, 3, 4].map((n) => (
          <div key={n} className="h-3 bg-muted" style={{ width: `${92 - n * 10}%` }} />
        ))}
      </div>
    </div>
  );
...
export default function ArticlePage() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      const { data: res } = await api.get(`/articles/${slug}`);
      return res.article;
    },
    enabled: !!slug,
    staleTime: 60_000,
    ...
    return (
      <PublicLayout>
        <div className="max-w-xl mx-auto p-8 text-center">
          <div className="font-heading font-black uppercase text-3xl mb-3">404</div>
          <p className="font-mono text-sm text-muted-foreground mb-4">Article not found.</p>
          <Link href="/" className="brutal-btn-primary text-[10px]">← Back home</Link>
        </div>
      </PublicLayout>
    );
  }
